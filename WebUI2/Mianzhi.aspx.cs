...
namespace WebUI2
{
    public partial class Mianzhi : System.Web.UI.Page
    {
        /// <summary>
        /// FaceSDK subscription key
        /// </summary>
        private readonly string _subscriptionKey = ConfigurationManager.AppSettings["FaceAPIKey"].ToString();
        private readonly string _AmlKey = ConfigurationManager.AppSettings["AMLAPIKey"].ToString();
        ...
        {
            //Catch();
            FaceVal x = await CallAmlService.CallAml();
            lblCap.Text = x.ScoredClass.ToString();
            Double p = x.Probability * 100;
            lblRes.Text = string.Format("{0}%", Math.Round(p,2).ToString());
        }
